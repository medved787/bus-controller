...
> Go-модуль и итоговый бинарник называются `bus-controller` (см. `backend/go.mod`, `backend/Dockerfile`). Контейнер в docker-compose при этом называется `port-monitor` — это старое имя сервиса, оставшееся в `docker-compose.yml`.
...
- `services.json` в репозитории — лишь пример/заготовка на 4 сервиса (`svc1`–`svc4`), два из которых (`svc2`, `svc3`) указывают на один и тот же `host:port` и имеют одинаковое имя `Service 2` — для реального использования конфиг нужно заменить своими сервисами.
