...
> **Альтернатива: AWX Webhook Service `GitLab`.** Вместо HMAC-подписи (`GitHub`-стиль выше) AWX поддерживает и вебхук-сервис `GitLab` — он проверяет подлинность запроса по статичному токену в заголовке (по умолчанию `X-Gitlab-Token`), без вычисления подписи тела. В этом случае `webhook.hmac_secret`/`webhook.hmac_header` в `services.json` можно оставить пустыми, а сам Webhook Key передать обычным заголовком через `webhook.headers` (`{"X-Gitlab-Token": "..."}`). Именно так настроен пример в `backend/services.json` — URL там вида `.../job_templates/{id}/gitlab/`.
...
- `services.json` в репозитории — пример/заготовка на 2 сервиса (`svc1` — TCP-проверка, `svc2` — HTTP-проверка), оба используют вебхук в GitLab-стиле (`X-Gitlab-Token`, см. выше). У обоих в примере `host` оставлен пустой строкой (`""`) — с таким конфигом `config.go` откажется стартовать («missing host/port»), поэтому перед запуском обязательно подставьте реальные адреса сервисов.
- Значение `paste_webhook_key_here` в заголовке `X-Gitlab-Token` — заглушка; замените его реальным Webhook Key из AWX перед использованием кнопки Restart.
- Секреты в закомментированном блоке `oauth2-proxy` в `docker-compose.yml` (`CHANGE_ME`, `CHANGE_ME_32_BYTE_BASE64_SECRET`) — тоже заглушки, обязательно замените их реальными значениями перед включением этого сервиса и продакшн-разворачиванием.
- Каталог `backend/web/` — файлы старого фронтенда, оставшиеся от разделения на `backend`/`frontend`; они больше не копируются в Docker-образ backend'а и не раздаются `main.go` (см. «Как это работает»), каталог не используется и его можно удалить.
